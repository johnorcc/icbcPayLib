...
class DefaultIcbcClient
{
     ...
     * @param $appId string APP的编号,应用在API开放平台注册时生成
     * @param $privateKey string APP应用私钥
     * @param $signType string 签名类型，CA -工行颁发的证书认证; RSA 表示RSAWithSha1; RSA2 表示RSAWithSha256;缺省为RSA
     * @param $charset string 字符集，仅支持UTF-8
     * @param $format string 请求参数格式，仅支持json
     * @param $icbcPublicKey string 网关公钥，必填
     * @param $encryptKey string AES加密密钥，缺省为空
     * @param $encryptType string 加密类型，当前仅支持AES加密，需要按照接口类型是否需要加密来设置，缺省为空
     * @param $ca string 当签名类型为CA时，通过该字段上送证书公钥，缺省为空
     * @param $password string 当签名类型为CA时，通过该字段上送证书密码，缺省为空
     */
    ...
    function execute($request, $msgId, $appAuthToken)
    {
        ...
        //解析响应
        $passed = IcbcSignature::verify($respBizContentStr, IcbcConstants::$SIGN_TYPE_RSA, $this->icbcPublicKey, $this->charset, $sign);
